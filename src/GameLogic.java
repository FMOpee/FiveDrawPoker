...
public class GameLogic implements GameLogicable{
    ...
    public boolean nextState(String[] messages) throws Exception {
        if(currentState==0) state0(messages);
        else if(currentState==1) state1(messages);
        else if(currentState==2) state2(messages);
        else if(currentState==3) state3(messages);
        else if(currentState==4) state4(messages);
        else state5(messages);
    ...
    private void state0(String[] messages){
        for (int i=0; i< 5; i++){
    ...
    private void state1(String[] messages) throws Exception {
    ...
    private void state2(String[] messages) throws Exception {
        randomSelect();
    ...
    private void state3(String[] messages){
        deck.shuffle();
        cpuHand.draw(deck,false);
        deck.shuffle();
    ...
    private void state4(String[] messages){
    ...
    private void state5(String[] messages) throws Exception {
